...
def transform_diagnosis(data):
  trajectories = {
    ...
    def get_category(code, trajectories=trajectories):
        ...
        for item, value in trajectories.items():
            ...
            if (re.sub('[^a-zA-Z]', '', code).upper()>value[0][0].upper()):
                ...
                pass
            elif (re.sub('[^a-zA-Z]', '', code).upper()==value[0][0].upper()) and (float(insert_decimal(re.sub('[^0-9]','', code), index=2))>=int(value[0][1:])):
                # example S21 > s00
                pass
            else:continue
    ...
    #             print(value[0], code, value[1])
                return "_".join(value)
            elif (re.sub('[^a-zA-Z]', '', code).upper()==value[1][0].upper()) and (int(float(insert_decimal(re.sub('[^0-9]','', code), index=2)))<=int(value[1][1:])):
                # example S21 > s00
    #             print(value[0], code, value[1])
                return "_".join(value)
            else:continue
