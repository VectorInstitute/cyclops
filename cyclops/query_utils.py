...
@query_params_to_type(Subquery)
def drop_attributes(t: Union[Select, Subquery, Table, DBTable], \
    drop_cols: Union[str, Column, List[Union[str, Column]]]) -> Select:
...
@query_params_to_type(Subquery)
def rename_attributes(t: Union[Select, Subquery, Table, DBTable], \
    d: dict) -> Select:
...
@query_params_to_type(Subquery)
def reorder_attributes(t: Union[Select, Subquery, Table, DBTable], \
    cols: List[Union[str, Column]], assert_same: bool = True) -> Select:
...
@query_params_to_type(Subquery)
def apply_to_attributes(t: Union[Select, Subquery, Table, DBTable], \
    cols: List[Union[str, Column]], fn: Callable) -> Select:
    """
...
def trim_attributes(t: Union[Select, Subquery, Table, DBTable], cols: List[Union[str, Column]]):
...
def equals_cond(col: Column, value: Any, lower: bool = True, \
    trim: bool = True, **kwargs: dict) -> BinaryExpression:
    ...
    **kwargs : dict, optional
    ...
    """
    return process_attribute(col, lower=lower, trim=trim, **kwargs) == \
        process_elem(value, lower=lower, trim=trim, **kwargs)
    

def string_format_cond(col: Column, value: Any, fmt: str, \
    to_str: bool = True, **kwargs: dict) -> BinaryExpression:
...
def substring_cond(col: Column, substring: Any, lower: bool = True, \
    **kwargs: dict) -> BinaryExpression:
    ...
    **kwargs : dict, optional
    ...
    sqlalchemy.sql.elements.BinaryExpression
        An expression representing where the condition was satisfied.
    """
    return string_format_cond(col, substring, '%%{}%%', \
        lower=lower, **kwargs)


def startswith_cond(col: Column, value: Any, lower: bool = True, \
    trim: bool = True, **kwargs: dict) -> BinaryExpression:
    ...
    **kwargs : dict, optional
    ...
    sqlalchemy.sql.elements.BinaryExpression
        An expression representing where the condition was satisfied.
    """
    return string_format_cond(col, value, '{}%%', \
        lower=lower, trim=trim, **kwargs)


def endswith_cond(col: Column, value: Any, lower: bool = True, \
    trim: bool = True, **kwargs: dict) -> BinaryExpression:
...
def in_list_condition(col: Column, lst: List[Any], lower: bool = True, \
    trim: bool=True, **kwargs: dict) -> BinaryExpression:
