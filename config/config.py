...
def read_config(file = False):
    ...
    parser.add('--pop_size', type=int, required=False, help='Total number of records to read from the database (0 - to read all)')
    parser.add('--filter_year', type=int, required = False, help='Select only records from before specified year')
    ...
    parser.add('--split_column',  type=str, required=False, help='Column we are use to split data into train, test, val')
    ...
    parser.add('--slice', type=str, required=False,
    ...
    parser.add('--report_path', type=str, required=False, help='Directory where to store html report?')
