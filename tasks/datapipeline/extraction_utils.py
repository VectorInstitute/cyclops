...
def get_scale(be_like, actual):
    ...
    if isinstance(be_like, str) & isinstance(actual, str):
        ...
        if any(item in be_like for item in multipliers) and any(item in actual for item in multipliers):
            # then adjust
            scale *= 1000 ** -multipliers.index(re.search('x10e\d+', actual)[0]) * 1000 ** multipliers.index(
                re.search('x10e\d+', be_like)[0])
            return scale
...
def numeric_categorical(item):
    ...
    item = '(' + item.replace('to', ' + ').replace('-',
                                                   ' + ') + ')/2'  # this neglects negative ranges. todo, find a fast regex filter
...
def convert(from_unit, to_unit):
    ...
    c_mat = np.concatenate(
        [np.logspace(-i * 3, (-i + len(prefixes) - 1) * 3, len(prefixes)).reshape(1, -1) for i in range(len(prefixes))],
        axis=0)
    ...
    for base_unit in ['g', 'mol', 'l']:  # this order is safe
        ...
        if (base_unit in from_unit) & (base_unit in to_unit):
            ...
            return c_mat[units.index(from_unit), units.index(to_unit)]  # scale is the result of this
        elif (base_unit in from_unit) + (base_unit in to_unit) == 1:
            raise Exception(f'Base units do not match for {from_unit} and {to_unit}')
    raise Exception(f"Either {from_unit} or {to_unit} are not multiples of ['g', 'mol', 'l']")
...
def transform_diagnosis(data):
    ...
    data = pd.concat((data, pd.get_dummies(data.loc[:, 'mr_diagnosis'].apply(get_category, args=(TRAJECTORIES,)),
                                           dummy_na=True, columns=TRAJECTORIES.keys(), prefix='icd10')), axis=1)
    return data
