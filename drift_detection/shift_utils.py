...
def get_dataset_hospital(datset,outcome,hospital,na_cutoff): 
    ...
    if datset == "covid":
        ...
        admin_data["admit_timestamp"].dt.date < datetime.date(2020, 3, 1), "encounter_id"
        ]
        ids_covid = admin_data.loc[
            admin_data["admit_timestamp"].dt.date > datetime.date(2020, 2, 28), "encounter_id"
        ]
        ...
        x_s = x_spl[0] ; x_t = x_spl[1]
        
    elif datset == "seasonal":
        ids_summer = admin_data.loc[
        (admin_data["admit_timestamp"].dt.month.isin([6,7,8]) & admin_data["admit_timestamp"].dt.year.isin([2018,2019])), "encounter_id"
        ]
        ids_winter = admin_data.loc[
        (admin_data["admit_timestamp"].dt.month.isin([12,1,2]) & admin_data["admit_timestamp"].dt.year.isin([2018,2019])), "encounter_id"
        ]
        ...
        (admin_data["hospital_id"].isin(["THPC","THPM"]) & admin_data["admit_timestamp"].dt.year.isin([2018,2019]) & 
        admin_data["admit_timestamp"].dt.month.isin([6,7,8])), "encounter_id"
        ]
        ids_academic = admin_data.loc[
        (admin_data["hospital_id"].isin(["SMH", "MSH","UHNTG", "UHNTW"]) & admin_data["admit_timestamp"].dt.year.isin([2018,2019]) & 
        admin_data["admit_timestamp"].dt.month.isin([6,7,8])), "encounter_id"
        ]
        ...
        (admin_data["admit_timestamp"].dt.month.isin([3,4,5,6,7,8]) & admin_data["admit_timestamp"].dt.year.isin([2019])), "encounter_id"
        ]
...
def import_dataset_hospital(datset,outcome,hospital,na_cutoff,shuffle=True):
    #get source and target data
    x_source, y_source, x_test, y_test, feats = get_dataset_hospital(datset, outcome, hospital, na_cutoff)
    
    #get train, validation and test set
    x_train, x_val, y_train, y_val = train_test_split(x_source, y_source, train_size=0.67)
...
def run_shift_experiment(shift, outcome, hospital, path, dr_technique, md_test, samples, dataset,sign_level,na_cutoff, random_runs=5,calc_acc=True):
    ...
    for rand_run in range(0,random_runs-1):
        ...
        (X_tr, y_tr), (X_val, y_val), (X_t, y_t), feats, orig_dims = import_dataset_hospital(shift, outcome, hospital,na_cutoff, shuffle=True)
...
def run_synthetic_shift_experiment(shift, outcome, hospital, path, dr_technique, md_test, samples, dataset, sign_level, na_cutoff, random_runs=5,calc_acc=True):
    # Stores p-values for all experiments of a shift class.
    samples_rands = np.ones((len(samples), random_runs)) * (-1)
    ...
    for rand_run in range(0,random_runs-1):
        ...
        (X_tr, y_tr), (X_val, y_val), (X_t, y_t), feats, orig_dims = import_dataset_hospital("baseline", outcome, hospital,na_cutoff, shuffle=True)
        ...
        for si, sample in enumerate(samples):
            ...
            # Detect shift.
            shift_detector = ShiftDetector(dr_technique, md_test, sign_level, red_model, sample, dataset)
            p_val, dist, red_dim, red_model, val_acc, te_acc = shift_detector.detect_data_shift(X_tr, y_tr, X_val, y_val, X_t_1[:sample,:], y_t_1[:sample], orig_dims)
