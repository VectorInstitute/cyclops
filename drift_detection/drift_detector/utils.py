...
class LKWrapper:
    def __init__(self, X_s, *, backend = 'tensorflow', p_val = 0.05, preprocess_x_ref = True, update_x_ref = None, preprocess_fn = None, 
    n_permutations = 100, var_reg = 0.00001, reg_loss_fn = lambda kernel: 0, train_size = 0.75, retrain_from_scratch = True, optimizer = None, 
    learning_rate = 0.001, batch_size = 32, preprocess_batch = None, epochs = 3, verbose = 0, train_kwargs = None, device = None, 
    dataset = None, dataloader = None, data_type = None, kernel_a = GaussianRBF(trainable=True), kernel_b = GaussianRBF(trainable=True), 
    eps = 'trainable', proj_type = 'ffnn'):
...
def recurrent_neural_network(model_name, input_dim, hidden_dim = 64, layer_dim = 2, dropout = 0.2, output_dim = 1, last_timestep_only = False):
    '''
...
def get_streams(X, y, admin_data, start_date, end_date, stride=1, window=1, ids_to_exclude=None, encounter_id='encounter_id', admit_timestamp='admit_timestamp'):
    '''
