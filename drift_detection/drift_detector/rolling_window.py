...
class RollingWindow:
    ...
    def rolling_window_mean(x: pd.DataFrame = None, window: int = 30):
        """
    ...
    def rolling_window_stdev(x: pd.DataFrame = None, window: int = 30):
        """
    ...
    def rolling_window_performance(
        ...
        while i + stat_window + lookup_window < len(X_test):
            ...
            X_next = pd.concat(
                X_test[i + lookup_window : i + lookup_window + stat_window]
            )
            ...
            y_next = pd.concat(
                y_test[i + lookup_window : i + lookup_window + stat_window]
            )
            y_next.index = ind
    ...
    def rolling_window_drift(
        ...
        while i + stat_window + lookup_window < len(X_test):
            ...
            X_next = pd.concat(
                X_stream[i + lookup_window : i + lookup_window + stat_window]
            )
            X_next = X_next[~X_next.index.duplicated(keep="first")]
            X_next = reshape_inputs(X_next, num_timesteps)
            ...
            (p_val, dist, val_acc, te_acc) = self.shift_detector.detect_data_shift(
