...
class GaussianDensity:
    ...
    def gradient_log_prob(self, X):
        ...
        self._check_fitted("The density must be fitted before sample probabilities can be taken")
        X = check_array(X,  ensure_2d=False, dtype=np.float)
        if X.ndim == 1:
            X = X.reshape(1, -1)
        grad_log_probs = np.empty_like(X)
        X = torch.from_numpy(X)
        for sample_idx, sample in enumerate(X):  # iterating over each sample to calc the gradient of the log prob.
            sample.requires_grad_(True)
            log_prob = self.density_.log_prob(sample)
            grad_log_probs[sample_idx] = torch.autograd.grad(log_prob, sample)[0]  # returns tuple with [0] as grad
...
class Knn:
    ...
    def __init__(self, n_neighbors=100):
        ...
        self.knn = NearestNeighbors(n_neighbors=self.n_neighbors, algorithm='kd_tree',
    ...
    def sample(self, n_samples, random_state=None, with_replacement=True):
        ...
        self._check_fitted()
        rng = check_random_state(random_state)
        sample_idxs = rng.choice(self.X_train_.shape[0], size=n_samples, replace=with_replacement)
    ...
    def conditional_sample(self, feature_idx, X):
        ...
        X_train_not_j = np.delete(self.X_train_, feature_idx, axis=1)  # removes the jth feature from the emperical dist
        X_not_j = np.delete(X, feature_idx, axis=1)  # removes the jth feature from the query data
        neighbor_idxs = self.knn.fit(X_train_not_j).kneighbors(X_not_j, return_distance=False)
