...
class GaussianDensity:
    ...
    def conditional_sample(self, x, feature_idx, n_samples=1, random_state=None):
        ...
        x: array-like, shape (n_features)
            The sample which we are going to be conditioning on. More specifically,
            we will be condtioning on the value of the jth feature in x.
        feature_idx: int
        ...
        random_state: int, RandomState instance, or None, optional (default=None)
            If int, then the random state is set using np.random.RandomState(int),
            if RandomState instance, then the instance is used directly,
            if None then a RandomState instance is used as if np.random() was called
    ...
    def _calculate_1d_guassian_conditional(
        ...
        # i.e. \Simga = [ [var(x_j), cov(x_j, x_{-j})],
        # [cov(x_{-j}, x_j), cov(x_j, x_j)] ]
...
class Knn:
    ...
    def conditional_sample(self, feature_idx, x):
        ...
        ----------
        feature_idx : int
            The index of the feature we are not conditioning on, i.e.
             the feature to be dropped when searching for
        ...
        neighborhood_conditional_samples :
            ...
            For example, neighborhood_conditional_samples[0] w
            ill have k (k=n_neighbors) rows, which correspond to the
            k neighbors we set out to find (k is set during this class initialization)
            in order of increasing distance
            from the search point (x), and the columns are
            the feature values for the neighbor.
