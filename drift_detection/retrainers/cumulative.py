        ...
        **kwargs
        ...
        while i+stat_window+lookup_window < len(data_streams['X']):
            ...
            if p_val < p_val_threshold:
                ...
                X_update_streams = pd.concat(data_streams['X'][max(int(i)-run_length,0):int(i)])
                ...
                if self.model_name in ["rnn","gru","lstm"]:
                    ...
                    if correct_only:
                        y_test_labels, y_pred_values, y_pred_labels = self.optimizer.evaluate(
                            update_loader, batch_size=1, n_features=input_dim, timesteps=num_timesteps
                        )
                        ...
                        X_update = X_update_streams.loc[X_update_streams.index.get_level_values(0).isin(encounter_ids)]
                        X_update = scale(X_update_streams)
                        X_update = process(X_update, aggregation_type, num_timesteps)   
            ...
            if p_val < p_val_threshold:
                if self.verbose:
                    print("P-value below threshold for ",data_streams['timestamps'][i+lookup_window],"-",data_streams['timestamps'][i+stat_window+lookup_window],"\tP-Value: ",drift_metrics['p_val'])
            else:
                i += stride
                run_length += stat_window
