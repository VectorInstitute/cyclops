...
The dataset contains 112,120 frontal-view X-ray images of 30,805 
unique patients with the text-mined fourteen disease image labels.
...
class NIHCXRDataset:
    ...
    def __init__(self, cfg_path: str = 'drift_detection/datasets/configs/nihcxr.yaml'):
        ...
        self.dataset = NIH_Dataset(self.image_path, self.csv_path,
                                   views=list(self.views), unique_patients=self.unique_patients, 
        ...
        self.metadata = pd.concat([self.metadata, pd.DataFrame(data=self.dataset.dataset.labels[indices, :], index=indices, columns=self.dataset.dataset.pathologies)], axis=1)
