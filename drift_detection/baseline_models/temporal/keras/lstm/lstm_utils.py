...
class MultitaskMetrics(keras.callbacks.Callback):
    ...
    def calc_metrics(self, data_gen, history, dataset, logs):
        ...
        for i in range(data_gen.steps):
            ...
            # decomp
            for (m, t, p) in zip(
                decomp_M.flatten(), decomp_t.flatten(), decomp_p.flatten()
            ):
                if np.equal(m, 1):
            ...
            if los_p.shape[-1] == 1:  # regression
                ...
                for (m, t, p) in zip(
                    los_M.flatten(), los_t.flatten(), los_p.reshape((-1, 10))
                ):
                    if np.equal(m, 1):
                        los_y_true.append(t)
                        los_pred.append(p)
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.early_stopping:
            ...
            if (pheno_max_auc > 0.75 and pheno_cur_auc < 0.73) and (
                ihm_max_auc > 0.85 and ihm_cur_auc < 0.83
            ):
                self.model.stop_training = True
