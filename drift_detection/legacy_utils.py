...
def import_dataset_hospital(
    outcome, train_hosp, test_hosp, features=None, shuffle=True
):
    data = pd.read_csv("/mnt/nfs/project/delirium/data/data_2019.csv")
...
def run_year_shift_experiment(
    ...
    for rand_run in range(0, random_runs - 1):
        ...
        (X_tr, y_tr), (X_val, y_val), (X_t, y_t), orig_dims = import_dataset_year(
            "los", year, features, shuffle=True
        )
...
def run_hosp_shift_experiment(
    ...
    for rand_run in range(0, random_runs - 1):
        ...
        for si, sample in enumerate(samples):
            ...
            red_model = None
            print(
                "Hospital %s: Random Run %s : Sample %s"
                % (str(test_hosp), rand_run, sample)
            )
